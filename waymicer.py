...
lenxm = len(mods[0])
lenym = len(mods[1])
stepx = res[0]/len(grid[0])/pow(2,lenxm)
stepy = res[1]/len(grid)/pow(2,lenym)
...
def xpos( x, offset):
    return stepx/2 + stepx*x + offset*res[0]/pow(2, lenxm)

def ypos( y, offset):
    return stepy/2 + stepy*y + offset*res[1]/pow(2, lenym)

template = 'bindsym SunProps+%s%s%s seat "-" cursor set %d %d'
for mx in range(0, 2**lenxm):
    for my in range(0, 2**lenym):
